...
import {showMobileExport} from './mobile.js';
...
let state={schemaVersion:3,projectId:'kotoba-closet',look:{...INITIAL_LOOK},focus:'top_blouse',studyLexeme:null,favorites:[],albums:[],learning:{},settings:{kana:true,reduced:false,mode:'light',slow:false},scene:'cafe'};
let history=[],future=[],question=null,answered=false,feedback='',category='all',favoritesOnly=false,voices=[],reviewMode=false,renderId=0,readyPlan=null,studyLevel='basic',learningTab='word',practiceMode='word';
...
function renderInventory(){
 ...
 $('#itemGrid').replaceChildren(...filtered.map(x=>{const card=el('div',undefined,`item ${state.look[x.slot]===x.id?'worn':''}`);const wear=button('',()=>equip(x.id),'wear-button');wear.dataset.itemId=x.id;wear.setAttribute('aria-label',`${x.slot==='hair'?'选择':state.look[x.slot]===x.id?'取下':'穿上'}${x.zh}`);wear.setAttribute('aria-pressed',String(state.look[x.slot]===x.id));const image=el('img');image.src=x.thumbnail;image.alt=x.zh;image.loading='lazy';image.decoding='async';image.fetchPriority='low';wear.append(image,el('strong',x.zh),el('small',x.jp));const fav=button(state.favorites.includes(x.id)?'♥':'♡',()=>{state.favorites=state.favorites.includes(x.id)?state.favorites.filter(id=>id!==x.id):[...state.favorites,x.id];save();renderInventory();},'fav');fav.setAttribute('aria-label',`收藏${x.zh}`);fav.setAttribute('aria-pressed',String(state.favorites.includes(x.id)));card.append(wear,fav);if(x.brand)card.append(el('span',BRANDS[x.brand].latin,'brand-tag'));return card;}));
...
function selectLearningTab(tab){learningTab=tab;document.querySelectorAll('[data-learning-tab]').forEach(button=>button.setAttribute('aria-selected',String(button.dataset.learningTab===tab)));document.querySelectorAll('[data-learning-panel]').forEach(panel=>panel.hidden=panel.dataset.learningPanel!==tab);}
function setPracticeMode(mode){practiceMode=mode;question=null;answered=false;feedback='';document.querySelectorAll('[data-practice-mode]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.practiceMode===mode)));renderQuestion();}
...
function renderScene(){
 const id=state.scene;$('#sceneSelect').value=id;const rows=SCENE_CONTENT.filter(row=>row.scene===id),brief=$('#sceneBrief'),vocabulary=$('#sceneVocabulary'),initial=0;
 ...
 const next=button(`换一句场景表达（1/${rows.length}）`,()=>{const current=(Number(brief.dataset.sceneIndex||0)+1)%rows.length;brief.dataset.sceneIndex=String(current);line.textContent=rows[current].jp;reading.textContent=rows[current].reading;translation.textContent=rows[current].zh;speakButton.onclick=()=>speak(rows[current].jp);next.textContent=`换一句场景表达（${current+1}/${rows.length}）`;},'text-btn');
...
function render(){const free=state.settings.mode==='free';document.body.classList.toggle('free-mode',free);document.body.classList.toggle('reduced',state.settings.reduced);$('#learnPanel').hidden=free;$('#challengeCard').hidden=false;$('#learningToggle').setAttribute('aria-pressed',String(!free));document.querySelectorAll('[data-mode]').forEach(b=>{b.classList.toggle('active',b.dataset.mode===state.settings.mode);b.setAttribute('aria-pressed',String(b.dataset.mode===state.settings.mode));});$('#kanaSetting').checked=state.settings.kana;$('#motionSetting').checked=state.settings.reduced;$('#slowSetting').checked=state.settings.slow;$('#undoBtn').disabled=!history.length;$('#redoBtn').disabled=!future.length;$('#albumCount').textContent=state.albums.length;renderInventory();renderStage();renderScene();if(!free){renderWord();renderQuestion();}}
function openAlbum(){const grid=$('#albumGrid');grid.replaceChildren(...[...state.albums].reverse().map(album=>{const card=el('article',undefined,'album-look'),name=el('input');name.value=album.name;name.maxLength=60;name.setAttribute('aria-label','穿搭名称');name.onchange=()=>{album.name=name.value.trim()||'我的穿搭';save();};const canvas=el('canvas');canvas.width=256;canvas.height=384;drawPlan(canvas,planFor(album.look,album.scene)).catch(()=>{});card.append(canvas,name,button('恢复穿搭',()=>{mutate(()=>{state.look=cleanLook(album.look);state.scene=album.scene;state.studyLexeme=null;state.focus=Object.values(state.look)[0]||'top_blouse';});$('#albumDialog').close();}),button('删除',()=>{state.albums=state.albums.filter(a=>a.id!==album.id);save();$('#albumCount').textContent=state.albums.length;openAlbum();},'text-btn'));return card;}));if(!state.albums.length)grid.append(el('p','保存第一套穿搭，让今天留下来。'));if(!$('#albumDialog').open)$('#albumDialog').showModal();}
...
async function importBackup(file){try{if(!file||file.size>1024*1024)throw Error('请选择1MB以内的备份文件');const next=normalizeSave(JSON.parse(await file.text()));state=next;history=[];future=[];question=null;reviewMode=false;save();render();toast('已恢复备份。');}catch(e){toast(`导入失败：${e.message}`);}finally{$('#importInput').value='';}}
...
document.querySelectorAll('[data-study-level]').forEach(control=>control.onclick=()=>{studyLevel=control.dataset.studyLevel;document.querySelectorAll('[data-study-level]').forEach(button=>button.setAttribute('aria-pressed',String(button===control)));renderWord();});
...
selectLearningTab('word');setPracticeMode('word');render();refreshVoices();document.body.dataset.appReady='true';
